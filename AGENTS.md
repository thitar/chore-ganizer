...
- **No lint or format npm scripts** currently exist in either package (`npm run lint`/`npm run format` are not defined) — there's no ESLint or Prettier config checked in for either `backend/` or `frontend/`. If you add one, wire it into CI too, since neither workflow runs linting or formatting. `.github/workflows/security.yml` provides CodeQL/`npm audit`/Gitleaks/Semgrep/Trivy security scanning; `.github/workflows/quality.yml` provides PR unit-test, typecheck, build, and Docker validation.
