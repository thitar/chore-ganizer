...
- **There is no CI/CD Docker publishing or deployment pipeline** — `.github/workflows/security.yml` provides security scanning, and `.github/workflows/quality.yml` provides PR unit-test, typecheck, build, and Docker validation. Neither workflow publishes images to `ghcr.io` or deploys. Don't assume a version bump alone gets a new image published anywhere; see `docs/OPERATIONS.md#version-bumps`.
