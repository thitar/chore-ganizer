...
- **Backend also has a real-DB integration suite mixed into the same `npm test` run — there is no separate `test:unit`/`test:integration` split.** `backend/src/__tests__/{assignments,recurring,templates,points,users}.test.ts` import the real `app` and real `prisma` client, log in via `supertest` against seeded users (`dad@home.local`, `alice@home.local`, etc. — see `backend/prisma/seed.ts`), and read/write the live DB in `beforeAll`. `backend/src/routes/__tests__/auth.routes.test.ts` mocks `services/auth.service` but still goes through the real `authenticate` middleware (`backend/src/middleware/auth.ts`), which queries `prisma.user.findUnique` directly — so it needs a working DB too, even though it looks fully mocked at a glance. `app.test.ts` is the one look-alike that's actually fine without a DB: it `require`s the real app but never issues a request that reaches Prisma, and Prisma 5's client is lazy (doesn't validate `DATABASE_URL` until the first query).
- **Bootstrap required before `npm test` will pass**, from `backend/` (running `jest`/`npx jest` from the repo root instead collects frontend Vitest files and Playwright specs too and prints a wall of unrelated FAILs — always run backend tests with cwd `backend/`):
  ```bash
  npm install
  ...
  ```
  `db push` creates/syncs `backend/prisma/dev.db` (SQLite path resolves relative to `schema.prisma`, not cwd — despite the `./dev.db` in the URL, it does **not** land at `backend/dev.db`) and regenerates the Prisma client; `prisma:seed` runs `prisma/seed.ts` to create the 4 seeded users the integration tests log in as. This is exactly what `.github/workflows/quality.yml` does for the `backend` CI job — it's not optional local-only setup.
  - **Without `DATABASE_URL` set and no seeded `dev.db`, this isn't just "6 suites fail"**: running the full suite (not a single file) leaves Jest hanging past its own "Ran all test suites" line instead of exiting — you need `npx jest --forceExit` to get a shell prompt back. Don't chase that hang as a bug; it means the DB bootstrap above wasn't run.
  - `DATABASE_URL` normally comes from `backend/.env` for local `npm run dev`, but that file is gitignored and not seeded by default — a fresh clone/worktree needs the commands above run explicitly at least once.
...
- **CI/CD publishes Docker images, but there is no deployment pipeline** — `.github/workflows/security.yml` provides security scanning, `.github/workflows/quality.yml` provides PR unit-test, typecheck, build, and Docker validation, and `.github/workflows/publish.yml` publishes the `backend`/`frontend`/`backup` images to `ghcr.io/thitar/` (tagged `:<version>` and `:latest`) on every push to `main`. There is no automated deployment — deploying is a manual `docker compose up` step; see `docs/OPERATIONS.md#version-bumps`.

### Monorepo Structure

Two independent npm packages, `backend/` and `frontend/`, each producing its own Docker image (`ghcr.io/thitar/chore-ganizer-{backend,frontend}:VERSION`) — built locally and CI-published to `ghcr.io` on every push to `main` (see above). Both `package.json` files must carry identical version numbers — see [docs/OPERATIONS.md#version-bumps](./docs/OPERATIONS.md#version-bumps). The root `package.json` exists only for Playwright e2e tooling and has its own independent (currently out-of-sync) version field — not part of that contract.
