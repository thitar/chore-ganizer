...
### Added
- Sessions are now persisted in the main SQLite database (`PrismaSessionStore` + new `Session` table) instead of `express-session`'s in-memory store — logins survive backend restarts/redeploys, and sessions are included in the existing DB backups

### Changed
- Default session lifetime raised from 7 to 30 days (`SESSION_MAX_AGE`)
...
### Changed
- Bumped `react` and `react-dom` from 18.2.0 to 19.2.8, and `@types/react`/`@types/react-dom` to match, in frontend
...
### Changed
- Bumped `typescript` (devDependency) from 5.9.3 to 7.0.2 in frontend
- Added `vite/client` to `tsconfig.json`'s `types`, required for TS 7's stricter side-effect-import resolution (CSS/font imports in `main.tsx`)

## [3.3.8] - 2026-08-03

### Changed
- Bumped frontend Docker base image from `nginx:1.25-alpine` to `nginx:1.31-alpine`
...
### Changed
- Bumped `lucide-react` from 0.577.0 to 1.28.0 (frontend)
...
### Fixed
- Restored overdue and historical chores in non-calendar chore views.
- Restored calendar date detail so every chore for a selected day, including completed chores, is visible.
...
### Added
- Unlockable Pong rewards: children unlock the game after earning the 10 Chores badge, while parents always have private access
...
### Changed
- Updated upgrade instructions in `OPERATIONS.md` to distinguish source builds from registry pulls
- Added env var diff command to `OPERATIONS.md` for checking new/removed variables after upgrades

## [3.2.1] - 2026-07-20

> Retroactive documentation of PRs #147–#158. This version was never deployed separately — the repo jumped from 3.2.0 directly to 3.2.2.
...
### Added
- ErrorDisplay component for consistent error handling across the frontend

### Changed
- Synchronized version numbers across all files to 2.1.8
- Improved auth session handling for better reliability

### Fixed
- Fixed lockout mechanism that could incorrectly track failed attempts
...
- Synchronized frontend version with backend (2.1.5)
...
### Added
- Added local CI/CD testing capability using [act](https://github.com/nektos/act) - allows running GitHub Actions workflows locally before pushing to GitHub
- Updated CI/CD Guide with detailed instructions for using act locally

## [2.1.2] - 2026-03-02

### Fixed
- Fixed "minimatch is not a function" error in CI/CD tests by using selective npm overrides (targeting only glob and ignore-walk packages) instead of global minimatch override, maintaining security patch for CVE-2024-4067
