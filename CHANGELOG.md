...
### Fixed
- Frontend's displayed `appVersion` (`window.APP_CONFIG.appVersion`, generated by `frontend/docker-entrypoint.sh`) no longer depends on `.env`'s `APP_VERSION` being manually kept in sync. It's now baked into the image from `frontend/package.json` at `docker build` time (`frontend/Dockerfile` writes `/etc/chore-ganizer/VERSION`), so a plain local `docker compose up --build -d` always shows the correct version with no separate `.env` sync step. Root cause: the old design injected the version from `.env` at container *start*, which silently drifted from what was actually built whenever `.env` wasn't hand-edited or `./docker-compose.sh` wasn't used.
