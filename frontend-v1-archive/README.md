# DEPRECATED — v1-archive of frontend

This directory contains the **legacy v2.1.10 frontend codebase** that was
superseded by the v1-rewrite (the `frontend/` directory at the repo root).
...
The rewrite was built alongside the legacy in `frontend-v2/` and
`frontend/` respectively, then promoted to `frontend/` when all 8 phases
...
- **Reference only.** Use git history (`git log -- frontend-v1-archive/`)
  if you need to see how a legacy feature was implemented.
- **Do NOT deploy from this directory.** The Dockerfile and
  `docker-compose.yml` at the repo root target the new `frontend/`.
