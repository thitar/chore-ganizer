...
- **`routes/`** — one file per resource: `health`, `auth`, `templates`, `assignments`, `users`, `recurring`, `occurrences`, `points`, `games` — mounted under `/api/*` in `routes/index.ts`. **`recurring.routes.ts` only covers CRUD on the recurrence *rule*** (`RecurringChore`) — generated `RecurringOccurrence` instances are listed and completed through `assignments.routes.ts`/`GET /api/assignments` and `occurrences.routes.ts`/`POST /api/occurrences/:id/complete`, not through `/api/recurring`. There is no `GET /api/recurring/occurrences` endpoint.
- **`services/`** — business logic (`assignment`, `auth`, `gamification`, `games`, `notification` + `notification.formatters`, `points`, `recurring`, `template`, `users`); routes call services directly — there is no separate controller layer in the current backend, routes are thin wrappers around service calls
...
- **`schemas/`** — Zod schemas consumed by `validate()` middleware (`assignment`, `games`, `points`, `template`, `auth`, `users`, `recurring`). All request-body-bearing routes now run `validate(schema)`; `occurrences.routes.ts` has no schema because its one route (`POST /:id/complete`) takes no request body, only a URL param.
...
- **`App.tsx`** — route definitions via React Router v6. All routes require auth (`ProtectedRoute`) except `/login`; `/templates`, `/recurring-chores`, `/assignments`, `/users` additionally require `requiredRole="PARENT"`. Other authenticated routes: `/` (dashboard), `/my-chores`, `/points`, `/calendar`, `/games`, `/profile`. Unknown paths redirect to `/`.
- **`api/`** — one file per domain (`auth`, `templates`, `assignments`, `users`, `recurring`, `points`, `calendar`, `games`), each built via `createApiClient()` in `frontend/src/lib/apiClient.ts`.
- **`hooks/`** — TanStack Query hooks per domain (`useAuth`, `useAssignments`, `useTemplates`, `useUsers`, `useRecurringChores`, `usePoints`, `useCalendar`, `useGames`), plus a standalone `useDismissableMenu` (click-outside/escape handling for the Manage dropdown/sheet — not a data hook).
- **Games UI** — `TopNav` and `BottomTabBar` show `/games` only when the current user can play at least one game (`games?.PONG.unlocked` / `games?.pong.unlocked` aliases). `GamesPage` renders one `<GameCard>` per `GAME_REGISTRY` entry (`frontend/src/games/registry.ts`) inside `AppShell`; each card owns its launch/canvas/leaderboard and submits the final score through the generic `useSubmitScore` hook (`useSubmitScore().mutateAsync({ gameId, score })` at `frontend/src/hooks/useGames.tsx:12`). `games/PongCanvas.tsx` and `games/SnakeCanvas.tsx` own their fixed-size HTML canvas, animation loop, controls (Pong: pointer paddle; Snake: swipe), and game-over callback. No pause control; scores never affect points.
...
- **GameHighScore** — one personal high score per user/game (`userId` + `game` unique), used for Pong's personal best and child leaderboard; cascades when its user is deleted
...
**Games endpoints** (`routes/games.routes.ts`): `GET /api/games/me` returns a per-game record map keyed by `gameId` (`PONG`, `SNAKE` — extensible to 5 via `GAME_DEFS`) plus lowercase aliases (`pong`, `snake`) for backward-compat with shipped clients; each entry has `{ unlocked, personalBest, leaderboard }`. For children, `leaderboard` is filtered to peers holding that game's `unlockBadge` (`PONG` → `ten-chores`, `SNAKE` → `twenty-chores`) and ordered desc; parents have `leaderboard: null` and are never ranked. Parents always have private access; children must hold the game's badge. `POST /api/games/:gameId/scores` is the generic score route (validated, high-score-only, 404 on unknown `gameId`); the legacy `POST /api/games/pong/scores` remains supported for backward-compat. Both `GET /me` and score routes require authentication and re-check eligibility server-side. Game scores write only `GameHighScore`; they never create `PointLog` entries or affect points, levels, or badges.
