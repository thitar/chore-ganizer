...
- **Games UI** — `TopNav` and `BottomTabBar` show `/games` only when the current user can play Pong. `GamesPage` renders the game card and child leaderboard inside `AppShell`; `games/PongCanvas.tsx` owns the fixed-size HTML canvas, animation loop, pointer paddle controls, and game-over callback. It has no pause control and submits the final score through `useSubmitPongScore`.
...
**Games endpoints** (`routes/games.routes.ts`): `GET /api/games/me` returns Pong availability, the current user's personal best, and (for children only) the child leaderboard. Parents always have private Pong access; children must have the `ten-chores` badge. `POST /api/games/pong/scores` accepts a validated score and records it only when it exceeds the user's existing best. Both endpoints require authentication, and the service repeats eligibility checks server-side. Game scores write only `GameHighScore`; they never create `PointLog` entries or affect points, levels, or badges.
