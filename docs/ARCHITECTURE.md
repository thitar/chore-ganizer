...
- **Games UI** — `TopNav` and `BottomTabBar` show `/games` only when the current user can play at least one game (`games?.pong.unlocked`, backward-compat alias for `PONG`). `GamesPage` renders the game card and child leaderboard inside `AppShell`; `games/PongCanvas.tsx` owns the fixed-size HTML canvas, animation loop, pointer paddle controls, and game-over callback. It has no pause control and submits the final score through `useSubmitPongScore`. The backend registry is expanding to `SNAKE` (see Games endpoints below); frontend migrates to `PONG`/`SNAKE` keys in #219.
