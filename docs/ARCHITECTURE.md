...
- **Games UI** — `TopNav` and `BottomTabBar` show `/games` only when the current user can play at least one game (`games?.PONG.unlocked` / `games?.pong.unlocked` aliases). `GamesPage` renders one `<GameCard>` per `GAME_REGISTRY` entry (`frontend/src/games/registry.ts`) inside `AppShell`; each card owns its launch/canvas/leaderboard and submits the final score through the generic `useSubmitScore({ gameId, score })` (`frontend/src/hooks/useGames.tsx`). `games/PongCanvas.tsx` and `games/SnakeCanvas.tsx` own their fixed-size HTML canvas, animation loop, controls (Pong: pointer paddle; Snake: swipe), and game-over callback. No pause control; scores never affect points.
