...
- All external actions pinned to immutable commit SHAs (supply-chain hardening). Use these exact pins:
    ...
    steps:
      ...
      - name: Build and push backend
        ...
        with:
          context: ./backend
          push: true
          tags: ghcr.io/thitar/chore-ganizer-backend:${{ steps.version.outputs.version }}
      ...
      - name: Build and push frontend
        ...
        with:
          context: ./frontend
          push: true
          tags: ghcr.io/thitar/chore-ganizer-frontend:${{ steps.version.outputs.version }}
      ...
      - name: Build and push backup
        ...
        with:
          context: ./backup
          push: true
          tags: ghcr.io/thitar/chore-ganizer-backup:${{ steps.version.outputs.version }}
...
- Produces: documented ghcr.io image names; compose remains functionally build-only.
...
- **Files:**
- Modify: `docs/OPERATIONS.md` (Version Bumps step 2; "From source" section; "From a pre-built registry image" section)
...
- [ ] **Step 2: Update "From source" section**

Ensure the "From source" section mentions all three images, not just two. Replace "Images are built locally from `./backend/Dockerfile` and `./frontend/Dockerfile`" with "Images are built locally from `./backend/Dockerfile`, `./frontend/Dockerfile`, and `./backup/Dockerfile`". Replace "rebuilds both images" with "rebuilds all three images".

- [ ] **Step 3: Rewrite "From a pre-built registry image" section**

Replace the current section (which misleadingly says `docker compose pull`) with:
...
- [ ] **Step 4: Update key_facts.md Docker section**
...
- [ ] **Step 5: Update FUTURE-ROADMAP.md**
...
```bash
rg -n "no CI/CD workflow|No workflow builds|built and tagged locally|no upstream registry|no registry pull step|no registry publishing|No Docker image publishing|you've published images|no CI/CD Docker publishing" AGENTS.md docs/ | grep -vE "superpowers/|project_notes/issues.md"
```
...
**Type consistency:** Image names `chore-ganizer-{backend,frontend,backup}` are identical across Tasks 1-3; SHA pins match verified `git ls-remote` commits (checkout v7, buildx v4, login v4, build-push v7).
