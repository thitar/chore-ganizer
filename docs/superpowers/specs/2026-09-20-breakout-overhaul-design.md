...
New `level: number`, starting at 1 — same shape as Space Invaders. Clearing every brick (`bricks.every(b => !b.alive)`) advances `level`, rebuilds a harder layout, respawns a single fresh ball, and awards a `LEVEL_CLEAR_BONUS × level` score bonus, instead of ending the run. Any in-flight capsules and the widen effect are cleared on level-up, same as on a life lost.
...
| Tough bricks (2-hit) | From `level ≥ TOUGH_BRICK_LEVEL` (e.g. 3), a random fraction of bricks require 2 hits | Real depth-add, not just repetition — `Brick.alive: boolean` becomes `Brick.hits: number`, destroyed at `hits ≤ 0` |
...
- `frontend/src/__tests__/BreakoutCanvas.test.tsx` (132 lines today) needs the same kind of rendering-assertion updates as the Space Invaders canvas tests required (any test asserting exact `fillRect` args for the single ball needs to account for `balls` being an array), plus new coverage for the level-up/life-lost flash reading directly off engine fields.
