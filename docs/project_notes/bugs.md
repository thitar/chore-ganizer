...
- **Root Cause**: (1) A TOCTOU race in `recurring.service.generateOccurrences()` — it reads existing occurrences then `createMany`s the missing ones; two concurrent `getAll()` calls (parallel jest workers, or two users loading the app at once) both compute the same `missingDates` and the second hits the `@@unique([recurringChoreId, dueDate])` constraint → P2002 → 500. (2) `StatusBadge` type advertises `PARTIALLY_COMPLETE` but has no rendering branch. (3) `scaffold.test.tsx` mocks `auth.api` via `{...actual, getCurrentUser, login, logout}` and omits `getAuthStatus` — the real one fired an XHR from the login page.
- **Solution**: (1) Serialized `generateOccurrences` with an in-process promise-chain lock (the app is a single backend container, so this fully eliminates the production race) and set `maxWorkers: 1` in `backend/jest.config.js` so the shared-`dev.db` integration suites can't race across worker processes. (`skipDuplicates: true` was tried first but Prisma 5.22 rejects it for SQLite at validation time.) (2) Added a display-only `PARTIALLY_COMPLETE` branch to `StatusBadge` ("Partially Complete", sky styling) and dim (no strikethrough) such pills on the calendar; per ADR-007 amendment this is presentation-only, no producer or semantics added. (3) Mocked `getAuthStatus` in `scaffold.test.tsx` and added `queryClient.clear()` in `beforeEach` (the shared cache with `staleTime: Infinity` made the new test flaky).
- **Prevention**: Any lazy-create-with-unique-index must serialize or tolerate the duplicate-key race — a read-then-write is not idempotent under concurrency just because the write is batched. Prisma's `skipDuplicates` is PostgreSQL/CockroachDB-only; don't reach for it on SQLite. When a `vi.mock` factory spreads `...actual`, audit every function the component under test calls — the mock must override each one or a real network call escapes. UI components that accept a status union must have a branch for every member of that union.
- **File**: `backend/src/services/recurring.service.ts`, `backend/jest.config.js`, `frontend/src/components/StatusBadge.tsx`, `frontend/src/pages/CalendarPage.tsx`, `frontend/src/api/calendar.api.ts`, `frontend/src/__tests__/scaffold.test.tsx`
...
### 2026-09-19 - Frontend's displayed appVersion drifts from actual deployed code

- **Issue**: After merging PR #249 (v3.12.0), `docker compose logs frontend` still showed `appVersion: "3.7.0"` even after the user rebuilt the container with `docker compose up --build -d` (confirmed via `docker inspect` that the image itself was freshly built).
- **Root Cause**: `frontend/docker-entrypoint.sh` generated `window.APP_CONFIG.appVersion` from the `VITE_APP_VERSION` env var at container **start**, which `docker-compose.yml` sourced from `.env`'s `APP_VERSION`. That value is only kept correct by remembering to run `./docker-compose.sh` (which auto-syncs it from `backend/package.json`) instead of plain `docker compose` — a fragile, easy-to-forget manual step disconnected from the actual build. A rebuild changes the served code but never touches `.env`, so the label can be stale indefinitely even on current code.
- **Solution**: Bake the version into the image at `docker build` time instead of injecting it at container start. `frontend/Dockerfile` now runs `node -p "require('./package.json').version" > /app/VERSION` in the build stage and copies it to `/etc/chore-ganizer/VERSION` in the runtime stage; `docker-entrypoint.sh` reads that file instead of `$VITE_APP_VERSION`. Removed the now-dead `VITE_APP_VERSION=${APP_VERSION}` line from `docker-compose.yml`'s frontend `environment` block. This guarantees the displayed version always matches what was actually built, with zero `.env` dependency.
- **Prevention**: For any "what's actually running" indicator, prefer baking the value into the image from the same source of truth used for the version bump (`package.json`), rather than threading it through an externally-set env var that has its own independent (and forgettable) sync step. Reserve runtime env injection for things that genuinely need to vary per-deployment without a rebuild.
- **File**: `frontend/Dockerfile`, `frontend/docker-entrypoint.sh`, `docker-compose.yml`
