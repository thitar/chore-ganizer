...
- **Status**: Completed
- **Description**: Updated the games route tests to use an explicit in-memory test session, mount the production CSRF middleware, and remove the login-shaped session helper; removed an unused service-test import. This clears the four CodeQL findings reported on the Pong PR without changing application behavior.
- **Verification**: Backend 292/292 tests passed, TypeScript build passed, and `git diff --check` passed.
- **URL**: https://github.com/thitar/chore-ganizer/pull/174
...
### 2026-09-17 — Issue #242: Breakout games-reward (3rd arcade game)

- **Status**: Completed
- **Description**: Shipped Breakout as the 3rd arcade game, unlocked by the existing `thirty-chores` badge (30 completed chores), closing the reservation noted in `docs/superpowers/specs/2026-07-30-games-rewards-design.md` and `frontend/src/games/registry.ts`. Backend: one-line `GAME_DEFS` addition in `backend/src/services/games.service.ts` — no new routes/schemas, the existing generic `/api/games/me` and `/api/games/:gameId/scores` cover it. Frontend: `frontend/src/games/breakout.ts` (pure deterministic engine mirroring `pong.ts` — 800x500 field, 5x10 brick grid, paddle/wall/brick AABB collision, single life, ends the run on either a missed ball or a fully cleared board) and `frontend/src/games/BreakoutCanvas.tsx` (mirrors `PongCanvas`'s RAF loop + pointer-drag paddle). Registered in `registry.ts` (`unlockLabel: '30 Chores'`). Version bumped to 3.10.0: initially targeted 3.9.0 since #245/#246 both claimed 3.8.0 off the same `main`, but both merged before this PR (landing as 3.8.0 and 3.9.0 respectively — the automated PR review caught the resulting merge conflict), so this PR was rebased and re-bumped to 3.10.0.
- **Notes**: `brickBounceAxis` resolves which face the ball hit primarily by which axis the ball is actually travelling into the brick along (gated on velocity direction vs. brick position), falling back to penetration-depth comparison only when neither/both axes qualify — the depth-only approach was tried first and found broken during self-review: a purely vertical serve (`vx===0`) has no horizontal component to ever flip, so it drilled straight through a column of bricks every single game. Fixed alongside an angled initial serve (`vx = 0.35×speed`) for the same reason. Verified via a bounded-frame smoke test (paddle never moves, ball must eventually miss). Tests: `breakout.test.ts`, `BreakoutCanvas.test.tsx` (mirroring the Pong suites), `GamesPage.test.tsx` extended (registry length 2→3, Breakout locked-state + submit-score coverage), backend `games.service.test.ts` extended (4 `toEqual` shapes + a 3rd `findMany` leaderboard-query assertion, per the issue's called-out gotcha). Full suites green: 388 backend / 254 frontend tests.

- **Date**: 2026-08-13
- **Description**: Implemented CI/CD Docker image publishing (design + plan in `docs/superpowers/specs/2026-08-13-ci-cd-image-publishing-design.md`). Added `.github/workflows/publish.yml` which builds and pushes `backend`, `frontend`, and `backup` images to `ghcr.io/thitar/` (tagged `:VERSION` + `:latest`) on every push to `main`, reading `APP_VERSION` from `backend/package.json` and failing if the frontend version mismatches. Documented the ghcr.io image names in `docker-compose.yml` comments (compose stays build-only). Updated `docs/OPERATIONS.md` (version-bump step 2 — publishing now automatic), `docs/project_notes/key_facts.md`, `docs/FUTURE-ROADMAP.md` (deferred row → shipped), and reconciled the stale "no publishing pipeline" claims in `AGENTS.md`, `docs/ARCHITECTURE.md`, and `docs/DOCKER-CONFIGURATION.md`. No `APP_VERSION` bump (release tooling only, no app-behavior change). Verifiable via `workflow_dispatch` on `main` after merge.
