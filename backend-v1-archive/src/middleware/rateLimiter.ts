...
const userRequestCounters: Map<number, UserRequestCounter> = new Map()
...
/**
 * Increment request count for a specific user
 */
export function incrementUserRequestCount(userId: number): void {
  const now = Date.now()
  const windowMs = parseInt(process.env.RATE_LIMIT_WINDOW_MS || '900000', 10)
  const existing = userRequestCounters.get(userId)

  if (!existing || now - existing.windowStart > windowMs) {
    userRequestCounters.set(userId, { count: 1, windowStart: now })
  } else {
    existing.count++
  }
}
...
/**
 * Get request count for all users
 */
export function getUserRequestCounts(): Array<{ userId: number; count: number; windowStart: string }> {
  const now = Date.now()
  const windowMs = parseInt(process.env.RATE_LIMIT_WINDOW_MS || '900000', 10)
  const result: Array<{ userId: number; count: number; windowStart: string }> = []

  for (const [userId, counter] of userRequestCounters.entries()) {
    // Only return users with activity in the current window
 ...
 */
export function getUserRequestCount(userId: number): { count: number; windowStart: string } | null {
  const counter = userRequestCounters.get(userId)
  if (!counter) return null

  const now = Date.now()
  const windowMs = parseInt(process.env.RATE_LIMIT_WINDOW_MS || '900000', 10)

  // If window expired, treat as no data
  if (now - counter.windowStart > windowMs) {
    return null
  }
...
/**
 * Reset all per-user request counters
 */
export function resetUserRequestCounts(): void {
  userRequestCounters.clear()
}
